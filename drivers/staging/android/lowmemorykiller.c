...
void tune_lmk_param(int *other_free, int *other_file, struct shrink_control *sc)
{
	...
	use_cma_pages = true; /* mtk platform is always true */
...
static int lowmem_shrink(struct shrinker *s, struct shrink_control *sc)
{
	...
	other_file = global_page_state(NR_FILE_PAGES) -
						...
						(si.freeswap >> 1) -  /* expand free size incase zram*/
    ...
    /* mtk orginial algorithm does not work well when Low memory 
     * zone has very low free memory, but file cache is still sufficient 
     * to go, let shrink to reclaim cache firstly instead of kill
     */
    ...
    /* no wait to compete the mutex if the task is dying 
     * to trigger lowmemorykiller to work
     */
	...
	for_each_process(tsk) {
        ...
        /* select exiting or fatal signaled process to kill take no
         * affection 
         */
        if (fatal_signal_pending(p) ||
            ((p->flags & PF_EXITING) &&
                test_tsk_thread_flag(p, TIF_MEMDIE))) {
            lowmem_print(2, "skip dying slowly process %d\n", p->pid);
            task_unlock(p);
            continue;
        }
#ifdef CONFIG_ANDROID_LOW_MEMORY_KILLER_AUTODETECT_OOM_ADJ_VALUES
